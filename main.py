...
cursor.execute('''
    ...
    VALUES 
        ('28 - MAKTAB', 'Chimyon', '+99899...', TRUE),
        ('15 - MAKTAB', 'Chimyon', '+99890...', FALSE);

    INSERT INTO teachers (first_name, last_name, email, contact_number, school_id)
    VALUES 
        ('Nodir', 'Aliyev', 'ali@example.com', '+99897...', 1),
        ('Gopir', 'Ndirov', 'nodir@example.com', '+99893...', 2);

    INSERT INTO students (first_name, last_name, dob, gender, school_id)
    VALUES 
        ('Ali', 'Aliyev', '2005-05-10', 'M', 1),
        ('Nodira', 'Nomonova', '2006-07-12', 'F', 2);
...
cursor.execute('''
    UPDATE schools SET name = '28 - MAKTAB' WHERE school_id = 1;
    UPDATE teachers SET email = first_name || teacher_id || '@gmail.com';  
    UPDATE teachers SET email = 'ali@example.com' WHERE teacher_id = 1;
    UPDATE classes SET name = '9-B' WHERE class_id = 1;
''')
